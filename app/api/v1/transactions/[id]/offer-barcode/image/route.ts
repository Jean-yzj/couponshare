...
// Streams the decrypted offer barcode — valid signed token AND re-validated
// authority (owner only after reveal; claimant = uploader anytime). Never cached.
export const GET = route(async (req, ctx) => {
  ...
  const isClaimant = t.claimantId === payload.uid;
  const isOwner = t.ownerId === payload.uid;
